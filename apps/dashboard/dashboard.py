...
def init_dashboard(server):
    ...
    dash_app.title = "Sales Prediction"
    ...
    def outlier(df):
        ...
        for i in column_name:
            ...
            df[i] = np.where(df[i] > upper_quartile, df[i].mean(),
                             np.where(df[i] < lower_quartile, df[i].median(), df[i]))
        return df
    ...
    @dash_app.callback(Output('line-1', 'figure'),
